...
def detect_asset_type(symbol: str) -> Tuple[str, str]:
    ...
    # Fallback
    logger.warning(f"Could not confidently detect asset type for {symbol}, defaulting to stock")
    return ('stock', symbol)
...
def format_pair_for_alpha_vantage(pair: str) -> Tuple[str, str]:
    ...
    raise ValueError(f"Invalid forex pair format: {pair}. Expected 6 letters or 'XAUUSD'")
